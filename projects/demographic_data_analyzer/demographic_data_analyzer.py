...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round((len(df[(df['salary'] == '>50K') & ((df['education'] == 'Bachelors') | (df['education'] == 'Masters') | (df['education'] == 'Doctorate') )])/ higher_education) * 100,1)
    ...
    rich_percentage = (num_min_workers / len(df[(df['hours-per-week'] == df['hours-per-week'].min())])) * 100

    # What country has the highest percentage  that earn >50K?
    highest_earning_country = (df[(df['salary'] == '>50K')])['native-country'].value_counts().index[0]
    highest_earning_country_percentage = round((len(df[(df['native-country'] == highest_earning_country) & (df['salary'] == '>50K')]) / len(df[df['salary'] == '>50K'])) * 100,1)
